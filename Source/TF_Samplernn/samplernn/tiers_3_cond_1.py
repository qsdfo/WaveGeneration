#### Utiliser le label de conditionnnement des samples à générer plutôt que de ceux déjà connus pour les big_frame et frame_level
...
class SampleRnnModel(object):
	...
	def _init_weigths(self):
		...
		# Conditionning
		self.big_cond_proj = tf.get_variable("big_cond_proj", [self., ], dtype=tf.float32)
	...
	def _create_network_BigFrame(self,
		...
		with tf.variable_scope('BigFrame_layer'):
			...
			with tf.variable_scope('Preprocess_audio_inputs'):
				...
				big_cond = tf.reshape(condition, [-1, num_time_frames, self.big_frame_size])
				big_cond_embed = tf.matmul(big_cond, self.big_cond_proj)
	...
	def _create_network_Frame(self,
		...
		with tf.variable_scope('Frame_layer'):
			...
			with tf.variable_scope("FRAME_RNN"):
				...
				import pdb; pdb.set_trace()
				# cell_input += self._embedd_condition(condition)
