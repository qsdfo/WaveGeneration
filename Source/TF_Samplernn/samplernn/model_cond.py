...
class SampleRnnModel_cond(object):
	def __init__(self, batch_size, big_frame_size, frame_size,
		...
		self.num_cond=5 	 	# Number of conditioning labels
		self.emb_cond_size=256
	...
	def _create_network_BigFrame(self,
		...
		with tf.variable_scope('BigFrame_layer'):
			...
			big_cond_reshape = tf.reshape(big_cond, [
														big_input_sequences_shape[0],
														big_input_sequences_shape[1] / self.big_frame_size,
														self.big_frame_size * self.emb_cond_size])
			...
			big_frame_cond_proj_weights = tf.get_variable(
				"big_frame_cond_proj_weights", [self.big_frame_size*self.emb_cond_size, self.big_frame_size], dtype=tf.float32)
			with tf.variable_scope("BIG_FRAME_RNN"):
				for time_step in range(num_steps):
					if time_step > 0: tf.get_variable_scope().reuse_variables()
					cell_input = big_input_frames[:, time_step, :]
					cell_input += math_ops.matmul(big_cond_reshape[:, time_step, :], big_frame_cond_proj_weights)
					(big_frame_cell_output, big_frame_state) = self.big_cell(cell_input, big_frame_state)
	...
	def _create_network_Frame(self,
		...
		with tf.variable_scope('Frame_layer'):
			...
			frame_cond_reshape = tf.reshape(frame_cond, [
															input_sequences_shape[0],
															input_sequences_shape[1] / self.frame_size,
															self.frame_size*self.emb_cond_size])
			...
			frame_cond_proj_weights = tf.get_variable(
				"frame_cond_proj_weights", [self.frame_size*self.emb_cond_size, self.dim], dtype=tf.float32)
			with tf.variable_scope("FRAME_RNN"):
				for time_step in range(num_steps):
					...
					cell_input += big_frame_outputs[:, time_step, :]
					# Conditioning
					cell_input += math_ops.matmul(frame_cond_reshape[:, time_step, :], frame_cond_proj_weights)
	...
	def _create_network_Sample(self,
		...
		with tf.variable_scope('Sample_layer'):
			...
			sample_cond_proj_weights = tf.get_variable(
				"sample_cond_proj_weights", [self.emb_cond_size, self.dim], dtype=tf.float32)
			...
			# Conditioning
			sample_cond_reshape = tf.reshape(sample_cond, [-1, self.emb_cond_size])
			cond_term = math_ops.matmul(sample_cond_reshape, sample_cond_proj_weights)
			frame_outputs_shape = frame_outputs.get_shape()
			import pdb; pdb.set_trace()
			out = out + frame_outputs + tf.reshape(cond_term, frame_outputs_shape)
	...
	def _create_network_SampleRnn(self,
		...
		with tf.name_scope('SampleRnn_net'):
			#embed conditions
			embedding_cond = tf.get_variable("embedding_cond", [self.num_cond, self.emb_cond_size])
			self.cond_embed = tf.squeeze(embedding_ops.embedding_lookup(embedding_cond, self.cond))
			...
			big_cond = self.cond_embed[:,:-self.big_frame_size,:]
			big_frame_outputs, final_big_frame_state = self._create_network_BigFrame(num_steps=big_frame_num_steps, 
			...
			frame_cond = self.cond_embed[:, self.big_frame_size-self.frame_size:-self.frame_size, :]
			frame_outputs, final_frame_state  = self._create_network_Frame(num_steps=frame_num_steps,
			...
			# Careful !! For the sample-level we don't use the same time horizon than for the audio input
			# Instead we use the present conditioning value
			sample_cond = self.cond_embed[:, self.big_frame_size:, :]
