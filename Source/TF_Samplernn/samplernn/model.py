...
class SampleRnnModel(object):
		...
		with tf.variable_scope('BigFrame_layer'):
			...
			with tf.variable_scope('Preprocess_audio_inputs'):
				...
				big_input_frames = tf.reshape(big_input_sequences,
					[big_input_sequences_shape[0], big_input_sequences_shape[1] / self.big_frame_size, self.big_frame_size])
			...
			with tf.variable_scope("Weights"):
				big_frame_proj_weights = tf.get_variable("big_frame_proj_weights", 
					[self.dim, self.dim * self.big_frame_size/self.frame_size], dtype=tf.float32)
	...
	def _create_network_SampleRnn(self,
		...
		with tf.name_scope('SampleRnn_net'):
			...
			big_frame_outputs, final_big_frame_state = self._create_network_BigFrame(num_steps = big_frame_num_steps, big_frame_state = train_big_frame_state, big_input_sequences = big_input_sequences)
