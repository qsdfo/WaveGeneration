...
class SampleRnnModel(object):
	...
	def _create_network_BigFrame(self,
		...
		num_time_frames = (self.seq_len-self.big_frame_size) / self.big_frame_size
		upsampling_ratio = self.big_frame_size/self.frame_size 
		...
		with tf.variable_scope('BigFrame_layer'):
			...
			with tf.variable_scope("BIG_FRAME_RNN"):
				# Rnn
				big_cell = tf.contrib.rnn.GRUCell(self.dim)
				output, state = tf.nn.dynamic_rnn(big_cell, big_input_frames, initial_state=big_frame_state, dtype=tf.float32)

			with tf.variable_scope("Projection_to_frame_level"):
				projected_dim = self.dim * upsampling_ratio
				big_frame_proj_weights = tf.get_variable("big_frame_proj_weights", [self.dim, projected_dim], dtype=tf.float32)

				output = tf.reshape(output, [-1, self.dim])
				big_frame_outputs = math_ops.matmul(output, big_frame_proj_weights)
				big_frame_outputs = tf.reshape(big_frame_outputs, [-1, num_time_frames, projected_dim])

				with tf.variable_scope("Reshape_upsampled"):
					big_frame_outputs = self._upsampling_reshape(big_frame_outputs, num_time_frames * upsampling_ratio, self.dim)
	...
	def _create_network_Frame(self,
		...
		num_time_frames = (self.seq_len-self.big_frame_size) / self.frame_size
		upsampling_ratio = self.frame_size # (/ 1)
		...
		with tf.variable_scope('Frame_layer'):
			...
			with tf.variable_scope("FRAME_RNN"):
				...
				frame_cell_proj_weights = tf.get_variable("frame_cell_proj_weights", [self.frame_size, self.dim], dtype=tf.float32)
				...
				output, state = tf.nn.dynamic_rnn(cell, cell_input, initial_state=frame_state, dtype=tf.float32)

			with tf.variable_scope("Projection_to_sample_level"):
				projected_dim = self.dim * upsampling_ratio
				# Weights
				frame_proj_weights = tf.get_variable("frame_proj_weights", [self.dim, self.dim * self.frame_size], dtype=tf.float32)
				
				output = tf.reshape(output, [-1, self.dim])
				frame_outputs = math_ops.matmul(output, frame_proj_weights)
				frame_outputs = tf.reshape(frame_outputs, [-1, num_time_frames, projected_dim])
				with tf.variable_scope("Reshape_upsampled"):
					frame_outputs = self._upsampling_reshape(frame_outputs, num_time_frames * upsampling_ratio, self.dim)   # Actually self.frame_size / 1 as the upsampling ratio of the last level is 1
 	...
 	def _create_network_Sample(self,
 		...
 		num_time_frames = (self.seq_len-(self.big_frame_size-self.autoregressive_order+1))
		...
		with tf.variable_scope('Sample_layer'):
			...
			sample_filter = tf.get_variable("sample_filter", sample_filter_shape, initializer=filter_initializer)
	...
	def _create_network_SampleRnn(self,
		...
		with tf.name_scope('SampleRnn_net'):
			...
			big_frame_outputs, final_big_frame_state = self._create_network_BigFrame(big_frame_state=train_big_frame_state, big_input_sequences=big_input_sequences)
			...
			frame_outputs, final_frame_state  = self._create_network_Frame(big_frame_outputs=big_frame_outputs, frame_state=train_frame_state, input_sequences=input_sequences)

			#sample
			sample_input_sequences = self.encoded_input_rnn[:, self.big_frame_size-self.autoregressive_order:-1, :]
			sample_output = self._create_network_Sample(frame_outputs, sample_input_sequences=sample_input_sequences)
	...
	def loss_SampleRnn(self,
		...
		with tf.name_scope(name):
			...
			# Train
			raw_output, final_big_frame_state, final_frame_state = self._create_network_SampleRnn(train_big_frame_state, train_frame_state)
