...
class SampleRnnModel(object):
	...
	def _init_weigths(self):
		...
		# Conditionning
		self.weights["big_cond_embed"] = tf.get_variable("big_cond_embed", [self.big_frame_size, self.big_frame_size], dtype=tf.float32)
		self.weights["frame_cond_embed"] = tf.get_variable("frame_cond_embed", [self.frame_size, self.rnns[0]], dtype=tf.float32)
		self.weights["sample_cond_embed"] = tf.get_variable("sample_cond_embed", [1, self.mlps[0]], dtype=tf.float32)
	...
	def _embedd_condition(self, condition):
		# Build mbedding for condition
	...
	def _create_network_BigFrame(self,
		...
		with tf.variable_scope('BigFrame_layer'):
			...
			with tf.variable_scope('Embed_condition'):
				big_cond = tf.reshape(condition, [-1, self.big_frame_size])
				big_cond_embed = tf.matmul(big_cond, self.weights["big_cond_embed"]) # Does not change matrix dim
				big_cond_embed = tf.reshape(big_cond_embed, [-1, num_time_frames, self.big_frame_size])

			with tf.variable_scope("BIG_FRAME_RNN"):
				next_states = []
				rnn_in = big_input_frames + big_cond_embed
	...
	def _create_network_Frame(self,
		...
		with tf.variable_scope('Frame_layer'):
			...
			with tf.variable_scope('Embed_condition'):
				frame_cond = tf.reshape(condition, [-1, self.frame_size])
				frame_cond_embed = tf.matmul(frame_cond, self.weights["frame_cond_embed"]) # Does not change matrix dim
				frame_cond_embed = tf.reshape(frame_cond_embed, [-1, num_time_frames, self.rnns[0]])
	...
	def _create_network_Sample(self,
		...
		with tf.variable_scope('Sample_layer'):
			...
			with tf.variable_scope("Embed_condition"):
				sample_cond = tf.reshape(condition, [-1, 1])
				sample_cond_embed = tf.matmul(sample_cond, self.weights["sample_cond_embed"]) # Does not change matrix dim
				sample_cond_embed = tf.reshape(sample_cond_embed, [-1, seq_len-self.big_frame_size, self.mlps[0]])
				out += sample_cond_embed
	...
	def _create_network_SampleRnn(self,
		...
		with tf.name_scope('SampleRnn_net'):
			...
			big_cond = tf.cast(self.cond, tf.float32)[:,:-self.big_frame_size]
			big_frame_outputs, final_big_frame_state = self._create_network_BigFrame(big_frame_states=train_big_frame_states, big_input_sequences=big_input_sequences, seq_len=seq_len, condition=big_cond)

			#frame 
			input_sequences = tf.cast(self.encoded_input_rnn, tf.float32)[:, self.big_frame_size-self.frame_size:-self.frame_size, :]
			frame_cond = tf.cast(self.cond, tf.float32)[:, self.big_frame_size-self.frame_size:-self.frame_size]
			frame_outputs, final_frame_state  = self._create_network_Frame(big_frame_outputs=big_frame_outputs, frame_states=train_frame_states, input_sequences=input_sequences, seq_len=seq_len, condition=frame_cond)

			#sample
			sample_input_sequences = self.encoded_input_rnn[:, self.big_frame_size-	self.autoregressive_order:-1, :]
			sample_cond = tf.cast(self.cond, tf.float32)[:, self.big_frame_size:] 	# At the sample level we pass the condition of the sample we want to generate
			sample_output = self._create_network_Sample(frame_outputs, sample_input_sequences=sample_input_sequences, seq_len=seq_len, condition=sample_cond)
