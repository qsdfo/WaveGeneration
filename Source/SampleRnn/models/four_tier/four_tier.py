...
Four-tier model, Quantized input
...
def get_args():
    ...
    parser.add_argument('--frame_size_mid', help='How many samples per frame',
                        type=check_positive, required=True)
    ...
    parser.add_argument('--length_sec', help='length of the generated file in seconds',
                        type=check_positive, required=False)
    ####################################
...
FRAME_SIZE_MID = args.frame_size_mid  # How many samples per frame
...
if GEN_FLAG:
    ...
    N_SEQS = args.nseqs  # Number of samples to generate every time monitoring.
    RESULTS_DIR = 'results_4t_longergen'
...
def big_frame_level_rnn(input_sequences, h0, reset):
    ...
    output = SampleRnn.lib.ops.Linear(
        ...
        BIG_DIM,
        DIM * BIG_FRAME_SIZE / FRAME_SIZE_MID,
        rnns_out,
        initialization='he',
        weightnorm=WEIGHT_NORM
    )
    output = output.reshape((output.shape[0], output.shape[1] * BIG_FRAME_SIZE / FRAME_SIZE_MID, DIM))
...
def mid_frame_level_rnn(input_sequences, other_input, h0, reset):
    """
    ...
    frames = input_sequences.reshape((
        input_sequences.shape[0],
        input_sequences.shape[1] // FRAME_SIZE_MID,
        FRAME_SIZE_MID
    ))
    ...
    # Handling SKIP_CONN
    if RNN_TYPE == 'GRU':
        rnns_out, last_hidden = SampleRnn.lib.ops.stackedGRU('MidFrameLevel.GRU',
                                                   N_RNN,
                                                   DIM,
                                                   DIM,
                                                   gru_input,
        ...
        rnns_out, last_hidden = SampleRnn.lib.ops.stackedLSTM('MidFrameLevel.LSTM',
                                                    N_RNN,
                                                    DIM,
                                                    DIM,
                                                    gru_input,
    ...
    split_ratio = (FRAME_SIZE_MID // FRAME_SIZE)
    output = SampleRnn.lib.ops.Linear(
        'MidFrameLevel.Output',
        DIM,
        split_ratio * DIM,
        rnns_out,
        initialization='he',
    ...
    output = output.reshape((output.shape[0], output.shape[1] * split_ratio, DIM))
...
if args.debug:
    ...
    h0_mid.tag.test_value = numpy.zeros((BATCH_SIZE, N_RNN, H0_MULT*DIM), dtype='float32')
...
input_sequences_mid = sequences[:, BIG_FRAME_SIZE-FRAME_SIZE_MID:-FRAME_SIZE_MID]
...
frame_level_outputs_mid, new_h0_mid = mid_frame_level_rnn(input_sequences_mid, big_frame_level_outputs, h0_mid, reset)

frame_level_outputs, new_h0 = frame_level_rnn(input_sequences, frame_level_outputs_mid, h0, reset)
...
###########
all_params = SampleRnn.lib.get_params(cost, lambda x: hasattr(x, 'param') and (x.param is True))
ip_params = SampleRnn.lib.get_params(ip_cost, lambda x: hasattr(x, 'param') and (x.param is True)
                           and 'BigFrameLevel' in x.name)
...
train_fn = theano.function(
    [sequences, big_h0, h0_mid, h0, reset, mask],
    [cost, new_big_h0, new_h0_mid, new_h0],
    updates=updates,
...
# Sampling at mid frame level
big_frame_level_outputs = T.matrix('big_frame_level_outputs')
mid_frame_level_generate_fn = theano.function(
    [sequences, big_frame_level_outputs, h0_mid, reset],
    mid_frame_level_rnn(sequences, big_frame_level_outputs.dimshuffle(0,'x',1), h0_mid, reset),
    on_unused_input='warn'
)

# Sampling at frame level
mid_frame_level_outputs = T.matrix('mid_frame_level_outputs')
frame_level_generate_fn = theano.function(
    [sequences, mid_frame_level_outputs, h0, reset],
    frame_level_rnn(sequences, mid_frame_level_outputs.dimshuffle(0,'x',1), h0, reset),
    on_unused_input='warn'
)
...
fixed_rand_h0_mid = numpy.random.rand(N_SEQS//2, N_RNN, H0_MULT*DIM)
fixed_rand_h0_mid -= 0.5
fixed_rand_h0_mid = fixed_rand_h0_mid.astype('float32')
...
def generate_and_save_samples(tag):
    ...
    h0_mid = numpy.zeros(
                    (N_SEQS-fixed_rand_h0_mid.shape[0], N_RNN, H0_MULT*DIM),
        dtype='float32'
    )
    h0_mid = numpy.concatenate((h0_mid, fixed_rand_h0_mid), axis=0)
    h0 = numpy.zeros(
    ...
    for t in xrange(BIG_FRAME_SIZE, LENGTH):
        ...
        if t % FRAME_SIZE_MID == 0:
            mid_frame_level_outputs, h0_mid = mid_frame_level_generate_fn(
                samples[:, t-FRAME_SIZE_MID:t],
                big_frame_level_outputs[:, (t / FRAME_SIZE_MID) % (BIG_FRAME_SIZE / FRAME_SIZE_MID)],
                h0_mid,
                numpy.int32(t == BIG_FRAME_SIZE)
            )
        ...
        if t % FRAME_SIZE == 0:
            frame_level_outputs, h0 = frame_level_generate_fn(
                ...
                mid_frame_level_outputs[:, (t / FRAME_SIZE) % (FRAME_SIZE_MID / FRAME_SIZE)],
                h0,
                numpy.int32(t == FRAME_SIZE_MID)
            )
...
def monitor(data_feeder):
    ...
    _h0_mid = numpy.zeros((BATCH_SIZE, N_RNN, H0_MULT*DIM), dtype='float32')
    ...
    for _seqs, _reset, _mask in _data_feeder:
        _cost, _big_h0, _h0_mid, _h0 = test_fn(_seqs, _big_h0, _h0_mid, _h0, _reset, _mask)
        _costs.append(_cost)
...
h0_mid = numpy.zeros((BATCH_SIZE, N_RNN, H0_MULT*DIM), dtype='float32')
...
if GEN_FLAG:
    ...
    SampleRnn.lib.load_params(args.checkpoint)
    print "Wall clock time spent before sampling started: {:.2f}mins"\
        .format((time()-exp_start)/60.)

    print "Sampling!",
    # Generate samples
    generate_and_save_samples('staticgeneration')
    print "Done!"
    ...
    while True:
        ...
        start_time = time()
        cost, big_h0, h0_mid, h0 = train_fn(seqs, big_h0, h0_mid, h0, reset, mask)
