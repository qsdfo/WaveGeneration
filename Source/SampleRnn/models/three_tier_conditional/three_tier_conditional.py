...
models/three_tier_conditional/three_tier_conditional.py --exp AXIS1 --seq_len 512 --big_frame_size 8 \
...
THEANO_FLAGS=mode=FAST_RUN,device=gpu1,floatX=float32 python -u \
models/three_tier_conditional/three_tier_conditional.py --exp AXIS1 --seq_len 512 --big_frame_size 8 \
--frame_size 2 --weight_norm True --emb_size 64 --skip_conn False --dim 32 \
--n_rnn 2 --rnn_type LSTM --learn_h0 False --q_levels 16 --q_type linear \
--batch_size 64 --generation False --debug
...
--frame_size 2 --weight_norm True --emb_size 64 --skip_conn False --dim 32 \
--n_rnn 2 --rnn_type LSTM --learn_h0 False --q_levels 16 --q_type linear \
--batch_size 128 --generation True --checkpoint path/to/checkpoint --nseqs 5 --conditioning_index path/to/conditioning/index
############################################################
...
def get_args():
    ...
    # Generation argument ##############
    parser.add_argument('--generation', help='generate or train',
                        type=t_or_f, required=True)
    parser.add_argument('--checkpoint', help='checkpoint path',
                        type=str, required=False)
    parser.add_argument('--nseqs', help='number of files to generate',
                        type=check_positive, required=False)
    parser.add_argument('--conditioning_index', help='length of the generated file in seconds',
                        type=check_positive, required=False)
    ####################################
...
if GEN_FLAG:
    ...
    RESULTS_DIR = 'results_3t_cond_longergen'
    FOLDER_PREFIX = RESULTS_DIR  # os.path.join(RESULTS_DIR, tag)
else:
...
from datasets.dataset_conditional import music_train_feed_epoch as train_feeder
from datasets.dataset_conditional import music_valid_feed_epoch as valid_feeder
from datasets.dataset_conditional import music_test_feed_epoch as test_feeder
...
def big_frame_level_rnn(input_sequences, conditional_input, h0, reset):
    """
    ...
    ###############################################################
    # Compute conditional influence
    cond_per_sample = lib.ops.Linear(
        'BigFrameLevel.ConditionalTermSample',
        COND_DIM,
        DIM,
        conditional_input,
        initialization='he',
    ...
    cond_per_sample_reshape = cond_per_sample.reshape((
        conditional_input.shape[0],
        conditional_input.shape[1] // BIG_FRAME_SIZE,
        BIG_FRAME_SIZE * DIM,
    ))

    conditional_term_per_frame = lib.ops.Linear(
        'BigFrameLevel.ConditionalTermFrame',
        DIM * BIG_FRAME_SIZE,
        BIG_FRAME_SIZE,
        cond_per_sample_reshape,
        initialization='he',
        weightnorm=WEIGHT_NORM
    )

    rnn_input = frames + conditional_term_per_frame
    ###############################################################
...
def frame_level_rnn(input_sequences, other_input, conditional_input, h0, reset):
    """
    input_sequences.shape: (batch size, n frames * FRAME_SIZE)
    other_input.shape:     (batch size, n frames, DIM)
    other_input.shape:     (batch size, n frames * FRAME_SIZE, COND_DIM)
    ...
    # Compute conditional influence
    cond_per_sample = lib.ops.Linear(
        'FrameLevel.ConditionalTermSample',
        COND_DIM,
        DIM,
        conditional_input,
        initialization='he',
    ...
    cond_per_sample_reshape = cond_per_sample.reshape((
        conditional_input.shape[0],
        conditional_input.shape[1] // FRAME_SIZE,
        FRAME_SIZE * DIM,
    ))

    conditional_term_per_frame = lib.ops.Linear(
        'FrameLevel.ConditionalTermFrame',
        DIM * FRAME_SIZE,
        DIM,
        cond_per_sample_reshape,
        initialization='he',
    ...
    gru_input = lib.ops.Linear(
        ...
        initialization='he',
        weightnorm=WEIGHT_NORM,
    ) + other_input + conditional_term_per_frame
...
def sample_level_predictor(frame_level_outputs, conditional_input, prev_samples):
    """
    ...
    if EMB_SIZE == 0:  # no support for one-hot in three_tier and one_tier.
        ...
        # (N_SEQS*N_FRAMES*FRAME_SIZE, FRAME_SIZE, Q_LEVELS)
    ...
    ###############################################################
    # Compute conditional influence
    FIRST_DIM = conditional_input.shape[0]

    cond_per_sample = lib.ops.Linear(
        'SampleLevel.ConditionalTermSample',
        COND_DIM,
        DIM,
        conditional_input,
        initialization='he',
    ...
    # For when we will factorize the first step
    cond_per_sample2 = lib.ops.Linear(
        'SampleLevel.ConditionalTermSample2',
        DIM,
        DIM,
        cond_per_sample,
        initialization='he',
    ...
    conditional_term = cond_per_sample2.reshape((FIRST_DIM * SEQ_LEN, DIM))

    out += frame_level_outputs + conditional_term
    ###############################################################
...
conditions = T.imatrix('conditions')  # One-hot vector, should not exit 8bit -> 256 categories ??
h0 = T.tensor3('h0')
...
if args.debug:
    ...
    conditions.tag.test_value = numpy.zeros((BATCH_SIZE, SEQ_LEN+OVERLAP), dtype='int32')
...
conditions_one_hot = lib.ops.T_one_hot(conditions, COND_DIM)
big_input_sequences = sequences[:, :-BIG_FRAME_SIZE]
big_conditions = conditions_one_hot[:, BIG_FRAME_SIZE:]      # On feed les conditions de ce qui arrive
input_sequences = sequences[:, BIG_FRAME_SIZE-FRAME_SIZE:-FRAME_SIZE]
frame_conditions = conditions_one_hot[:, BIG_FRAME_SIZE:]
target_sequences = sequences[:, BIG_FRAME_SIZE:]

target_mask = mask[:, BIG_FRAME_SIZE:]

big_frame_level_outputs, new_big_h0, big_frame_independent_preds = big_frame_level_rnn(big_input_sequences, big_conditions, big_h0, reset)

frame_level_outputs, new_h0 = frame_level_rnn(input_sequences, big_frame_level_outputs, frame_conditions, h0, reset)
...
###########
all_params = lib.get_params(cost, lambda x: hasattr(x, 'param') and (x.param is True))
ip_params = lib.get_params(ip_cost, lambda x: hasattr(x, 'param') and (x.param is True)
                           and 'BigFrameLevel' in x.name)
...
#     [sequences, conditions, big_h0, reset, mask],
#     [ip_cost, new_big_h0],
#     updates=ip_updates,
...
#     [sequences, conditions, big_h0, h0, reset, mask],
...
big_frame_level_generate_fn = theano.function(
    [sequences, conditions, big_h0, reset],
    big_frame_level_rnn(sequences, conditions_one_hot, big_h0, reset)[0:2],
    on_unused_input='warn'
)
...
frame_level_generate_fn = theano.function(
    [sequences, conditions, big_frame_level_outputs, h0, reset],
    frame_level_rnn(sequences, big_frame_level_outputs.dimshuffle(0,'x',1), conditions_one_hot, h0, reset),
    on_unused_input='warn'
)
...
sample_level_generate_fn = theano.function(
    [frame_level_outputs, conditions, prev_samples],
    ...
    for t in xrange(BIG_FRAME_SIZE, LENGTH):
        ...
        if t % BIG_FRAME_SIZE == 0:
            big_frame_level_outputs, big_h0 = big_frame_level_generate_fn(
                ...
                conditioning[:, t-BIG_FRAME_SIZE:t],
                big_h0,
...
def monitor(data_feeder):
    ...
    for _seqs, _conds, _reset, _mask in _data_feeder:
        _cost, _big_h0, _h0 = test_fn(_seqs, _conds, _big_h0, _h0, _reset, _mask)   
        _costs.append(_cost)
...
if GEN_FLAG:
    ...
    conditionings = [numpy.loadtxt(e, dtype='int8', delimiter=";") for e in CONDITIONING_FILES]
    # Generate samples
    ...
    while True:
        ...
        seqs, conds, reset, mask = mini_batch

        ############################################################
        ############################################################
        # T T  E E  S S  T T
        # shape_conds = seqs.shape + (COND_DIM,)
        # conds = numpy.random.randint(low=0, high=1, size=shape_conds, dtype='int8')
        ############################################################
        ############################################################
        ...
        ############################################################
        # T T  E E  S S  T T
        # cost, big_h0, h0 = train_fn(seqs, conds, big_h0, h0, reset, mask)
        cost = 1.0
        ############################################################
        ############################################################
